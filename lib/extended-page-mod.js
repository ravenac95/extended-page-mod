...
const ExtendedPageMod = Loader.compose(EventEmitter, {
  ...
  _onContent: function _onContent(window) {
    ...
    console.log("EXCLUDING DUDE");
    console.log(JSON.stringify(this.excludeURLs));
    ...
    console.log("EXCLUDING DUDEasdf asdf asdf ");
    ...
    try {
      ...
      for (let rule in RULES)
        if (RULES[rule].test(document.URL)) {
          if (window.__virtruContentInjected) {
            return;
          }
          window.__virtruContentInjected = true;
